...
words, MLSAG should be used to sign multiple inputs, but it is currently
not the cas of Monero, where the inputs are signed one by one.
...
This has one unfortunate effect that `rows` is always equal to 2 and
...
For ring size = 3 the matrix M looks like this:
|------------------------|------------------------|------------------------|
...
def gen_first_c_and_key_images(message, rv, pk, xx, kLRki, index, dsRows, rows, cols):
    """
    ...
    # This loops runs currently only once (see the file header for explanation)
    ...
    # This runs once as well (see above) for commitments
    for i in range(dsRows, rows):
        alpha[i] = crypto.random_scalar()
        # L = alpha_i * G
        aGi = crypto.scalarmult_base(alpha[i])
        # for some reasons we omit calculating R here, which seems
        # contrary to the paper, but it is in Monero official client
...
def gen_mlsag(message, pk, xx, kLRki, index, dsRows):
    """
    ...
    c_old, Ip, alpha = gen_first_c_and_key_images(
...
def generate_mlsag_full(
    ...
    return gen_mlsag(message, M, sk, kLRki, index, rows)
...
def generate_mlsag_simple(message, pubs, in_sk, a, cout, kLRki, index):
    ...
    return gen_mlsag(message, M, sk, kLRki, index, dsRows)
